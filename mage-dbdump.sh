...
while getopts ":drzehfFai" OPTION; do
  case $OPTION in
    a)
      banner
      exit 0
...
[[ "$OPT_i$OPT_d$OPT_r" == "" ]] && usage && exit 1
...
MYSQL_ARGS="--socket=$DBHOST -u $DBUSER $DBNAME"
